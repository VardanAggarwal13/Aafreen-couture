        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#FAF7F2] border-b border-[#DDD2C5] text-[#8A6A55] text-[11px] uppercase tracking-wider font-semibold">
              ...
              <th className="text-left px-6 py-4 hidden md:table-cell">Display Order</th>
          ...
          <tbody className="divide-y divide-[#EAE2D7]">
            {categories.map((cat) => (
              <tr key={cat._id} className="hover:bg-[#FAF7F2]/60 transition-colors">
                <td className="px-6 py-4 text-[#2E221C] font-serif font-medium text-base">{cat.name}</td>
                <td className="px-6 py-4 text-[#8A6A55] text-xs font-mono hidden sm:table-cell">{cat.slug}</td>
                <td className="px-6 py-4 text-[#8A6A55] font-medium hidden md:table-cell">{cat.displayOrder}</td>
